...
public class NetworkUtils {
    ...
    private static final String MIN_VOTE_COUNT_VALUE="500";

    public static final int POPULARITY = 0;
    public static final int TOP_RATED = 1;

    public static URL buildURLToVideos (int id,String lang){
        URL baseURLVideos=null;
        Uri uri = Uri.parse(String.format(BASE_URL_VIDEOS, id)).buildUpon()
                .appendQueryParameter(PARAMS_API_KEY,API_KEY)
                .appendQueryParameter(PARAMS_LANGUAGE,lang)
                .build();
        try {
            baseURLVideos = new URL(uri.toString());
        } catch (MalformedURLException e) {
            e.printStackTrace();
        }
        return baseURLVideos;
    }

    public static URL buildURLToReviews (int id,String lang){
        URL baseURLReview=null;
        Uri uri = Uri.parse(String.format(BASE_URL_REVIEWS, id)).buildUpon()
                .appendQueryParameter(PARAMS_API_KEY,API_KEY)
                .appendQueryParameter(PARAMS_LANGUAGE,lang)
                .build();
        try {
            baseURLReview = new URL(uri.toString());
        } catch (MalformedURLException e) {
            e.printStackTrace();
        }
        return baseURLReview;
    }
